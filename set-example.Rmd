...
```{r, eval = F}
set500 <- replicate(500, set_game(), simplify = F) # list of 1000 set games
save(set500, file = "500_set_games.Rdata")
```

```{r}
load("500_set_games.Rdata")
```
...
set_stats <- tibble(data = set500, cards_left = res)
```

purrr version:
```{r}
set_stats <- tibble(data = set500) %>%
